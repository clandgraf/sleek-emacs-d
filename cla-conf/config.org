...
#+BEGIN_SRC emacs-lisp
  (global-set-key (kbd "<f9>") #'backward-sexp)
  (global-set-key (kbd "<f10>") #'forward-sexp)
  (global-set-key (kbd "S-<f10>") #'mark-sexp)
...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "C-c e") 'insert-char)
#+END_SRC
...
;  (defun cla/org-journal-path ()
;    (format-time-string "~/OneDrive/journal/%Y-%m.org" (current-time)))
;  (setq org-capture-templates
;        `(("t" "Todo" entry (file+headline "~/OneDrive/todo/todo.org" "Tasks")
;           "* TODO %?\n  %i\n  %a")
;          ("j" "Journal" entry (file+olp+datetree ,(function cla/org-journal-path))
;           nil
;           :empty-lines 1)))
  (setq org-agenda-include-diary t)
  (setq calendar-date-style 'iso)
  (setq diary-file "~/ownCloud/Documents/emacs-diary")
  (push "~/ownCloud/Documents/orga/agenda.org" org-agenda-files)
#+END_SRC

Custom Figures for my journal entries

#+begin_src emacs-lisp
  (defun cla/encode-image-b64 (source)
  ...
  (use-package org-special-block-extras
    ...
    (org-defblock myfigure
      (caption nil)
      "Custom environment for HTML figures"
      (let* ((images (split-string raw-contents))
             (embed nil))
        (shr-dom-to-xml
         `(figure ()
                  (div ()
                       ,@(--map `(img ((src . ,(if embed
                                                   (cla/encode-image-b64 it)
                                                 it))))
                                images))
...
On macOS by default option will be handled by the operating system. On
the one hand, option is used to input special characters, on the other
hand, it's a good option for inputting meta.
...
#+BEGIN_SRC emacs-lisp
  ...
  (defhydra hydra-insert-cedm ()
    "Insert CEDM Entities"
    ("c" cedm-insert-change-request "Change Request")
    ("p" cedm-md-insert-person "Person")
    ("o" cedm-insert-object "Object"))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (defhydra hydra-consult-ripgrep ()
    "consult-ripgrep variants"
    ("g" consult-ripgrep "Consult Ripgrep in current project")
    ("." consult-ripgrep-at-point "Consult Ripgrep at point"))
  ...
  (use-package consult
    ...
    :bind (("C-x b" . consult-buffer)
           ...
           ("M-s g" . hydra-consult-ripgrep/body)
           ("C-<f3>" . consult-ripgrep-at-point))
    :config
